...
flowchart LR
    ...
    MQTT --> Transport[Internal transport abstraction\nlocal/in-memory first]
...
| Internal transport abstraction | Carries raw MQTT messages between services. The first implementation should use a local/in-memory placeholder before RabbitMQ or another broker is introduced. |
