...
flowchart LR
    ...
    MQTT --> Gateway[device-gateway-service]
...
| `mqtt-protocol-service` | Subscribes to telemetry topics, validates topic structure, extracts identifiers, and creates the internal telemetry envelope. |
| `device-gateway-service` | Receives envelopes, validates required fields, identifies supported payloads, parses M2000 input-register telemetry, and writes logs. |
