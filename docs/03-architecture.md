...
flowchart LR
    ...
    PUSR --> MQTT[mqtt-protocol-service<br/>MQTTnet server]
...
| `mqtt-protocol-service` | Hosts the MQTTnet broker, receives device publishes, validates topic structure, extracts identifiers, detects payload encoding, and creates `RawMqttMessage` records. |
